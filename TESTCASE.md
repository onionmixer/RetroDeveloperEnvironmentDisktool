...
README.md를 기준으로 `rdedisktool`이 제공하는 기능 중 read-only 이미지(XSA) 관련 흐름을 제외한 모든 기능을 검증하는 수동 테스트 시나리오다. 각 시나리오는 서로 독립적으로 실행할 수 있으며, 공통 준비 과정을 한 번만 수행하면 된다. 모든 단계는 단순 명령 실행에 그치지 않고, `list`/`info`/`validate`/`cmp` 등을 통해 결과 상태를 검증해야 한다.

## 공통 준비
1. `rdedisktool` 바이너리가 `PATH`에 있거나 절대 경로로 호출 가능해야 한다.
2. 테스트용 호스트 파일은 `tests/fixtures/` (또는 임시 작업 디렉터리) 아래에 저장한다.
   - `HELLO.BAS`, `NEWPROG.BIN`, `README.TXT`, `CHAPTER1.TXT`, `DRAGON.COM`, `SHOOTER.COM`, `PATCH.BIN`, `PLAYER.BIN` 등은 1KB 이하의 더미 데이터를 `xxd -r -p` 또는 `printf`로 생성해도 무방하다.
...
## 시나리오 4. 포맷 변환 및 다중 플랫폼 검증
- **목적**: Apple II 섹터 순서 변환과 MSX 이미지 형식 변환 기능(`convert`)을 검증하고 변환 전후 데이터를 cross-check 한다.
- **커버리지**: `convert` 명령, Apple DO↔PO 섹터 순서 변환, MSX DSK↔DMK 형식 변환, 변환 후 `info`/`list`/`validate` 검증.
- **참고**: Apple II의 DO↔PO 변환은 섹터 순서만 변경하며, 파일시스템(DOS 3.3/ProDOS)은 변환되지 않는다.
...
## 시나리오 5. MSX XSA 압축/복원 및 read-only 검증
...
- **준비**: 시나리오 4에서 생성한 `msx.dsk` (PATCH.BIN 포함)을 사용하거나 동일한 데이터가 있는 MSX-DOS 이미지를 준비한다.
...
각 시나리오는 Apple II(DOS/ProDOS)와 MSX(DSK/DMK/XSA)의 수정 가능·불가능 흐름 전체를 아우르며, 특히 XSA(read-only) 시나리오에서는 쓰기 작업이 실패하는지도 함께 검증한다. 필요 시 각 시나리오 종료 후 생성한 임시 파일을 삭제하여 깨끗한 상태를 유지한다. 모든 작업은 결과 상태를 재차 검증해 실제 사용 환경에서도 안전하게 도구를 활용할 수 있음을 입증해야 한다.
